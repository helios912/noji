...
export function digitsCounter() {
    ...
    function digitsCountersInit(digitsCountersItems) {
        ...
        if (digitsCounters.length) {
            ...
            digitsCounters.forEach((digitsCounter) => {
                // Обнулення
                if (digitsCounter.hasAttribute('data-fls-digcounter-go'))
                    return;
    ...
    function digitsCountersAnimate(digitsCounter) {
        ...
        const duration = parseFloat(digitsCounter.dataset.flsDigcounterSpeed)
            ? parseFloat(digitsCounter.dataset.flsDigcounterSpeed)
            : 1000;
        const startValue = parseFloat(digitsCounter.dataset.flsDigcounter);
        const format = digitsCounter.dataset.flsDigcounterFormat
            ? digitsCounter.dataset.flsDigcounterFormat
            : ' ';
        ...
        const step = (timestamp) => {
            ...
            digitsCounter.innerHTML =
                typeof digitsCounter.dataset.flsDigcounterFormat !== 'undefined'
